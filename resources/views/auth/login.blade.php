...
<p class="text-slate-400 text-xs mb-5">Silakan login untuk mengelola laundry.</p>

@if(session('success'))
    <div class="mb-4 p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl text-xs flex items-center gap-2"><i data-lucide="check" class="h-4 w-4 shrink-0"></i><span>{{ session('success') }}</span></div>
@endif
@if($errors->any())
...
<form method="POST" action="{{ route('login.attempt') }}" class="space-y-4">
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input type="text" name="username" value="{{ old('username') }}" autofocus autocomplete="username" placeholder="username"
    ...
    <div>
        ...
        <div class="relative">
            ...
            <input type="password" name="password" id="loginPass" autocomplete="current-password" placeholder="••••••••"
            ...
            <button type="button" onclick="togglePass('loginPass', this)" class="absolute inset-y-0 right-0 flex items-center pr-3.5 text-slate-500 hover:text-slate-300"><i data-lucide="eye" class="h-4 w-4"></i></button>
        </div>
    </div>

    <div class="flex items-center justify-between">
        <label class="flex items-center gap-2 cursor-pointer select-none">
            <input type="checkbox" name="remember" value="1" class="w-4 h-4 rounded text-teal-600 bg-slate-950 border-slate-850">
            <span class="text-xs text-slate-350">Ingat saya</span>
        </label>
        <a href="{{ route('password.forgot') }}" class="text-xs text-accent hover:text-accent/90 font-semibold">Lupa password?</a>
    </div>
    ...
    <button type="submit" class="w-full py-3 bg-accent hover:bg-accent-hover text-white font-bold rounded-xl transition-all duration-200 shadow-lg hover:-translate-y-0.5 text-sm flex items-center justify-center gap-2">
        <i data-lucide="log-in" class="h-4.5 w-4.5"></i><span>Masuk</span>
    </button>
</form>
